...
def build_formatted_docx(spec: dict) -> BytesIO:
    ...
    for line in lines:
        ...
        # Roman numeral de-duplication — runs at ANY indent level so it catches
        # both "(b) (i)" at indent 1 AND wrongly-labelled "(b) (ii)" at indent 2.
        ...
        if _mcd:
            ...
            # Strip parent letter whenever it has already appeared — either as a
            # standalone "(b)" context line or in a previous combined label.
            _parent_shown = (
            ...
            if _parent_shown:
                ...
                # First combined hit — mark parent as seen; keep full combined
                # label only if there was no preceding standalone context line.
                _seen_roman_d[qnum].add(_parent_d)
        elif indent_level == 1:
            # Plain "(b)" context label — record it so any subsequent "(b) (x)"
            # lines (at any indent level) strip the redundant parent letter.
